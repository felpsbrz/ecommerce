...
async function carregarPedidos() {
  ...
  try {
    const res = await fetch('api.php?endpoint=get_orders&user_id=' + userId);
...
async function verPedido(id) {
  ...
  try {
    const res = await fetch('api.php?endpoint=get_order_items&order_id=' + id);
    const data = await res.json();
    ...
    document.getElementById('modalOrderTotal').textContent = parseFloat(total || 0).toFixed(2).replace('.', ',');
