...
<div class="form-group">
  ...
  <input 
    ...
    required
    value="<?= htmlspecialchars($telefoneUsuario) ?>"
  >
</div>
...
function calcTotal(cart) {
    return cart.reduce((acc, p) => acc + (parseFloat(p.price_promo || p.price) * p.qtd), 0);
}
...
document.getElementById('formPedido').addEventListener('submit', async e => {
    ...
    formData.append('endpoint', 'create_order');
    formData.append('total', total);
    formData.append('payment_method', payment_method + ' (' + payment_time + ')');
    formData.append('address', address);
    formData.append('items', JSON.stringify(cart));

    try {
        const res = await fetch('api.php?endpoint=create_order', {
            method: 'POST',
            body: formData
        });
        ...
        if (data.success) {
            ...
            cart.forEach(p => {
                msg += `- ${encodeURIComponent(p.name)} x${p.qtd} — R$ ${(parseFloat(p.price_promo || p.price) * p.qtd).toFixed(2).replace('.', ',')}%0A`;
            });
